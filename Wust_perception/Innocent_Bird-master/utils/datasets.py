...
class LoadWebcam:  # for inference
    ...
    def __next__(self):
        ...
        if self.pipe == 0:  # local camera
            ret_val, img0 = self.cap.read()
            # Image_Size = (img0.shape[:2])[::-1]
            # map1, map2 = cv2.initUndistortRectifyMap(self_parameter, distortion, np.eye(3),
            #                                          self_parameter, Image_Size, cv2.CV_16SC2)
            # img0 = cv2.remap(img0, map1, map2, interpolation=cv2.INTER_LINEAR,
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        for i, s in enumerate(sources):
            ...
            assert cap.isOpened(), 'Failed to open camera %s' % s
